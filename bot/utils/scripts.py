    ...
    def decorator(func):
        async def wrapped(client: Client, message: Message):
            if message.text and len(message.text.split()) == 1:
                # Проверьте, что message существует перед его изменением
                if message:
                    await message.edit(f"<emoji id=5210952531676504517>❌</emoji>{text}")
                else:
                    logger.error("Message is not provided for editing in with_args")
            else:
...
# Инициализация web_options как объекта ChromeOptions
web_options = ChromeOptions()

# Установка параметров для WebDriver
...
mobile_emulation = {"deviceMetrics": device_metrics, "userAgent": user_agent}

web_options.add_experimental_option("mobileEmulation", mobile_emulation)
web_options.add_argument("--headless")
web_options.add_argument("--log-level=3")
...
@contextmanager
def create_webdriver(seleniumwire_options=None):
    # Убедимся, что опции передаются только один раз
    if seleniumwire_options:
        driver = webdriver.Chrome(service=ChromeService(ChromeDriverManager().install()), seleniumwire_options=seleniumwire_options)
    else:
        driver = webdriver.Chrome(service=ChromeService(ChromeDriverManager().install()), options=web_options)
